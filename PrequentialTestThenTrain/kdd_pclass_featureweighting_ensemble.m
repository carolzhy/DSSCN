clear
clc
load('/home/visitor-2/DSSCN/dataset/KDD/kdd10persenshuff3dsscn.mat');
sourcedata=data;
...
for k=1:chunk_size:nData
    ...
    if ensemble==0
        ...
        for k1=1:size(Datatest,1)
            ...
            ysem_softmax = ysem;
            ysem_softmax = exp(ysem_softmax-max(ysem_softmax,[],2));
            ysem_softmax = ysem_softmax./sum(ysem_softmax,2);

            residual_error(res_error_count) = 1 - ysem(trueclasslabel);
            residual_error_softmax(res_error_count) = 1 - ysem_softmax(trueclasslabel);
        ...
        C(counter)=totalrule;   %node
        D(counter)=totalparameters; 
        %
        
        E(counter)=size(network,1); %Layer
        
        PRED = ensembleoutputtest-1;
        EVALU(counter,:)= Evaluate((Data(:,43)),PRED);
