...
input_pruning=0;%choose one to active feature selection scenario
subset=ninput;
CBtrain=Data_training;%[gaussiantraindata gaussiantrainlabel];
CBtest=Data_testing;%[gaussiantestdata gaussiantestlabel];
%CB=[CBtrain;CBtest];
        ...
        for hh=1:ensemble-l-1
          ...
          merged_list(end+1,2)=hh;
           ...
           break
                                        ...
                                        del_list=[];
                                          misclassification=zeros(noutput,size(network,1));
